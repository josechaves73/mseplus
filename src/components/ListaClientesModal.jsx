...
const ListaClientesModal = ({ isOpen, onClose }) => {
  ...
  const fetchClientes = async (search = '', order = 'asc') => {
    ...
    try {
      ...
      const url = `http://localhost:4000/api/clientes${queryString ? `?${queryString}` : ''}`;
  ...
  const handleConfirmDelete = async () => {
    if (selectedIndex >= 0 && filteredClientes[selectedIndex]) {
      ...
      try {
        const response = await fetch(`http://localhost:4000/api/clientes/${clienteSeleccionado.codigo}`, {
          method: 'DELETE',
        });
